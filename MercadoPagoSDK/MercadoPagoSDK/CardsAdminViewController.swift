...
open class CardsAdminViewController: MercadoPagoUIScrollViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func deleteCardAlertView(card: Card, message: String) {
        ...
        alert.addAction(UIAlertAction(title: PXStrings.no_string.PXLocalized, style: UIAlertActionStyle.cancel, handler: nil))
        alert.addAction(UIAlertAction(title: PXStrings.yes_string.PXLocalized, style: UIAlertActionStyle.default, handler: { (_) -> Void in
            self.callback(card)
        }))
