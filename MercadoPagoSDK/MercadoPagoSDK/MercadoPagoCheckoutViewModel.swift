...
open class MercadoPagoCheckoutViewModel: NSObject, NSCopying {
    ...
    public func updateCheckoutModel(paymentMethodSearch: PaymentMethodSearch) {
        ...
        if totalPaymentMethodsToShow == 0 {
            self.errorInputs(error: MPSDKError(message: PXStrings.error_title.PXLocalized, errorDetail: "No se ha podido obtener los métodos de pago con esta preferencia".localized, retry: false), errorCallback: { () in
            })
