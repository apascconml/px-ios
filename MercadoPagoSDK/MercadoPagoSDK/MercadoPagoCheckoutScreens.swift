...
final class PXCircularTransitionAnimator: NSObject, UIViewControllerTransitioningDelegate {
    private let circularTransition = PXCircularTransition()
    func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return circularTransition.make(targetPoint: source.view.center, color: ThemeManager.shared.successColor(), transitionMode: .present)
    }
}

final class PXCircularTransition: NSObject {

    enum CircularTransitionMode: Int {
        case present, dismiss, pop
    }

    fileprivate var circle: UIView = UIView()
    ...
    fileprivate var circleColor: UIColor = UIColor.white
    fileprivate var duration: TimeInterval = 0.3
    fileprivate var transitionMode: CircularTransitionMode = .present

    func make(targetPoint: CGPoint, color: UIColor, transitionMode: CircularTransitionMode) -> PXCircularTransition {
        self.startingPoint = targetPoint
...
extension PXCircularTransition: UIViewControllerAnimatedTransitioning {
...
extension MercadoPagoCheckout {
    ...
    func showBusinessResultScreen() {
        ...
        let circularTransitionAnimator = PXCircularTransitionAnimator()
        congratsViewController.transitioningDelegate = circularTransitionAnimator
        congratsViewController.modalPresentationStyle = .custom

        self.viewModel.pxNavigationHandler.navigationController.present(congratsViewController, animated: true, completion: nil)
