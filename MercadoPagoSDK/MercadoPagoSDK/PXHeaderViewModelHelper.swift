    ...
    func labelTextHeader() -> NSAttributedString? {
        if isAccepted() {
            if self.paymentResult.isWaitingForPayment() {
                return PXStrings.success_header_title.PXLocalized.toAttributedString(attributes: [NSAttributedStringKey.font: Utils.getFont(size: PXHeaderRenderer.LABEL_FONT_SIZE)])
            } else {
        ...
        if !preference.showLabelText {
            ...
            return NSMutableAttributedString(string: PXStrings.error_header_title.PXLocalized, attributes: [NSAttributedStringKey.font: Utils.getFont(size: PXHeaderRenderer.LABEL_FONT_SIZE)])
        }
